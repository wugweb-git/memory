...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (path === '/api/ingest/soul' && req.method === 'POST') {
      rateLimit(req, { key: 'ingest-soul', limit: 180 });
      enforcePayloadLimit(req);
      validateSpiritNotePayload(req.body || {});
      const r = await ingestSoul(req);
      return res.status(r.code).json(r.body);
    }
    if (path === '/api/ingest/pulse' && req.method === 'POST') {
      rateLimit(req, { key: 'ingest-pulse', limit: 180 });
      enforcePayloadLimit(req);
      validatePulsePayload(req.body || {});
    ...
    if (path === '/api/telemetry-config' && req.method === 'GET') { rateLimit(req, { key: 'telemetry-config', limit: 120 }); const r = await listTelemetryConfig(req); return res.status(r.code).json(r.body); }
