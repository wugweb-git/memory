...
export function SignalTimeline({ signals, loading }: SignalTimelineProps) {
  ...
  if (!signals || signals.length === 0) {
    return (
      <div className="h-32 flex flex-col items-center justify-center border border-dashed border-slate-800 rounded-lg text-text-tertiary bg-bg-primary/40">
          ...
          <div key={signal.id} className="p-3 hover:bg-slate-800/30 transition-colors flex items-center justify-between group">
            <div className="flex items-center gap-3">
              <div className={`p-1.5 rounded-full bg-slate-800 ${
                ...
                signal.type === 'learning' ? 'text-purple-400' : 'text-text-tertiary'
              }`}>
            ...
            <div className="flex items-center gap-4">
              <div className="text-right">
                ...
                <p className="text-2xs text-slate-600 uppercase font-bold">Rel. Intensity</p>
              </div>
              
              <div className="px-2 py-0.5 rounded-full bg-slate-800 border border-slate-700 text-2xs text-text-tertiary font-bold tracking-widest uppercase">
