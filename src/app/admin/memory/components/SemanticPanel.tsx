    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          {data.verification_status === 'unverified' && (
            <div className="px-2 py-0.5 rounded bg-orange-600 text-2xs font-bold text-white uppercase tracking-[0.2em]">
              UNVERIFIED
            </div>
          )}
        </div>
      ...
      <div className="grid grid-cols-2 gap-6">
        ...
        <div className="space-y-3">
          <h4 className="text-2xs uppercase font-bold text-slate-500 flex items-center gap-2 tracking-widest">
          ...
          <div className="space-y-2">
            {data.entities.map((ent, i) => (
              <div key={i} className="px-3 py-2 bg-slate-800/20 rounded border border-slate-700/30 flex items-center justify-between group hover:border-purple-500/30 transition-all">
                ...
                <span className="text-2xs text-slate-500 uppercase tracking-tighter font-mono">{ent.type}</span>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {data.intents.map((intent, i) => (
                <div key={i} className="px-2 py-0.5 rounded border border-blue-500/20 text-blue-400 bg-blue-500/5 text-2xs font-bold tracking-tight uppercase">
                  @{intent.intent}
                </div>
              ))}
              {data.topics.map((topic, i) => (
                <div key={i} className="px-2 py-0.5 rounded border border-slate-700 text-slate-400 bg-white/5 text-2xs font-bold tracking-tight uppercase">
                  # {topic.topic}
                </div>
              ))}
            </div>
              ...
              {data.relationships && data.relationships.length > 0 ? (
                <div className="space-y-4 pt-2">
                  {data.relationships.map((rel, i) => (
                    <div key={i} className="flex items-center gap-2 text-2xs text-slate-400">
