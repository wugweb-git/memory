...
export function SemanticPanel({ data, loading }: SemanticPanelProps) {
  ...
  if (!data) {
    return (
      <div className="h-48 flex flex-col items-center justify-center border border-dashed border-slate-800 rounded-lg text-text-tertiary bg-bg-primary/40">
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          {data.fallback ? (
            <div className="px-2 py-0.5 rounded border border-amber-500/30 text-2xs font-bold text-amber-500 bg-amber-500/5 uppercase tracking-widest flex items-center gap-1.5">
              <ShieldAlert className="w-2.5 h-2.5" />
              Fallback Mode
            </div>
          ) : (
            <div className="px-2 py-0.5 rounded border border-emerald-500/30 text-2xs font-bold text-emerald-500 bg-emerald-500/5 uppercase tracking-widest flex items-center gap-1.5">
        ...
        <span className="text-2xs text-slate-600 font-mono font-bold">CONF: {(data.confidence * 100).toFixed(0)}%</span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-6">
            ...
            {data.entities.map((ent, i) => (
              <div key={i} className="px-3 py-2 bg-slate-800/20 rounded border border-slate-700/30 flex items-center justify-between group hover:border-purple-500/30 transition-all">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-xs text-slate-200 group-hover:text-text-primary transition-colors capitalize font-medium">{ent.name}</span>
                </div>
        ...
        <div className="space-y-6">
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {data.intents.map((intent, i) => (
                <div key={i} className="px-2 py-0.5 rounded border border-blue-500/20 text-accent bg-blue-500/5 text-2xs font-bold tracking-tight uppercase">
                  @{intent.intent}
                </div>
              ))}
              {data.topics.map((topic, i) => (
                <div key={i} className="px-2 py-0.5 rounded border border-slate-700 text-text-tertiary bg-secondary text-2xs font-bold tracking-tight uppercase">
                  # {topic.topic}
                </div>
              ))}
            </div>
          ...
          <div className="space-y-3">
            ...
            <div className="p-3 bg-bg-primary/40 border border-slate-800/50 rounded-xl min-h-[120px] shadow-inner">
                  ...
                  {data.relationships.map((rel, i) => (
                    <div key={i} className="flex items-center gap-2 text-2xs text-text-tertiary">
                      ...
                      <div className="flex-1 border-t border-dashed border-slate-800 relative h-0">
                        <span className="absolute -top-2 left-1/2 -translate-x-1/2 px-1 bg-[#121212] text-2xs uppercase tracking-tighter text-blue-500 font-black whitespace-nowrap">
                          {rel.type}
                        </span>
                      </div>
