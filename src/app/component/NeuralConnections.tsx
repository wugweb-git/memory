    ...
    <section className="space-y-10 w-full" aria-label="Neural connections status">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-2">
        <div className="kinetic-text">
          ...
          <p className="text-2xs text-text-tertiary font-bold mt-1 uppercase tracking-[0.3em] opacity-60">External Identity Matrix — 1-Way Sync</p>
        </div>
        <div className="px-5 py-2 rounded-full bg-accent/5 border border-accent/20 text-2xs font-black text-accent flex items-center gap-3 uppercase tracking-widest shadow-sm">
      ...
      <div className="glass-panel p-8 rounded-[2rem] border border-border-secondary shadow-xl relative overflow-hidden">
        <div className="flex items-center justify-between text-2xs font-black text-text-tertiary uppercase mb-4 tracking-widest ">
      ...
      <ul role="list" className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            ...
            >
              <div className="flex items-center justify-between mb-8">
                ...
                <div className="flex flex-col items-end gap-1.5">
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-2xs font-black tracking-widest uppercase ${STATUS_COLORS[node.status]}`}>{node.status}</span>
                    <span className={`w-1.5 h-1.5 rounded-full ${STATUS_DOT[node.status]}`} />
                  </div>
                  <span className="text-2xs text-text-disabled font-mono font-bold uppercase tracking-tighter  opacity-60">
                    {node.lastActivity}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="kinetic-text">
                  ...
                  <p className="text-2xs font-black text-text-tertiary uppercase tracking-[0.2em]  opacity-60">{node.category} Target</p>
                </div>
                <div className="flex items-center justify-between p-4 rounded-2xl bg-bg-secondary/50 border border-border-primary group-hover:bg-black group-hover:text-bg-primary transition-all duration-500">
                  <span className="text-2xs font-black font-mono uppercase tracking-widest">{node.metric}</span>
        ...
        <li>
          ...
          >
            ...
            <span className="text-2xs font-black text-text-tertiary uppercase tracking-[0.3em] group-hover:text-accent transition-colors">Attach_Node</span>
          </button>
        </li>
      </ul>
      ...
      <div className="glass-panel p-10 rounded-[3rem] border border-border-secondary relative overflow-hidden group">
        ...
        <h3 className="text-2xs font-black tracking-[0.4em] text-accent mb-6 uppercase flex items-center gap-4 kinetic-text">
          Sync_Protocol_Architecture <Share2 size={16} />
        </h3>
        <p className="text-base text-text-secondary leading-relaxed max-w-2xl font-medium mb-8  tracking-tight">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
          {[
            ...
            <div key={stat.label} className="border-l-2 border-border-primary pl-6 hover:border-accent transition-colors">
              <dt className="text-2xs font-black text-text-disabled tracking-[0.2em] uppercase mb-2">{stat.label}</dt>
              <dd className="text-sm font-black text-text-primary tracking-tighter ">{stat.val}</dd>
            </div>
          ))}
        </div>
      </div>
