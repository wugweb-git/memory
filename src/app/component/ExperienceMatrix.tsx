  ...
  return (
    <div className="space-y-12 w-full" aria-label="Experience matrix and case studies">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 px-2">
        ...
        {selectedIndustry && (
          <motion.div
            ...
            className="px-4 py-2 rounded-full bg-accent text-2xs font-black text-bg-primary uppercase tracking-[0.2em] shadow-xl"
          >
      ...
      {loading ? (
        ...
        <p className="text-sm text-text-tertiary  px-2">No experience sections yet. Add profile sections with type &quot;experience&quot;.</p>
      ) : (
          ...
          {filteredStudies.map((study, idx) => (
            ...
            >
              <div className="flex flex-wrap items-center gap-3 mb-4">
                ...
                <span className="text-2xs text-text-disabled font-mono">{study.company} · {study.date}</span>
              </div>
              ...
              <div className="flex items-start gap-2 p-4 rounded-xl bg-accent/5 border border-accent/20">
                ...
                <p className="text-xs text-text-secondary ">{study.perspective2026}</p>
              </div>
              <div className="flex flex-wrap gap-2 mt-4">
                {study.industries.map((ind) => (
                  <span key={ind} className="text-2xs font-black px-2 py-1 rounded-full bg-secondary border border-border-secondary text-text-tertiary uppercase">
                    {ind}
                  </span>
                ))}
              </div>
