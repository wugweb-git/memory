          ...
          {waitingForAI &&
            (
              <div className="loading">
                <img alt="image" src='/1484.gif'></img>
              </div>
            )}
        </>
        <>
          {messages.length == 0 &&
            (
              <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'center' }}>
                <img alt="image" style={{ width: "25%", marginBottom: "2%" }} src='/MongoDB_White.svg' />
                <span style={{ marginBottom: '2%', fontSize: '40px', justifySelf: 'center' }}>+</span>
                <img alt="image" style={{ width: "8%", marginBottom: "2%" }} src='/openAI.svg' />
              </div>
            )
          }
        </>
            ...
            <div key={m.id} className="flex gap-3 my-4 text-gray-600 text-sm flex-1">
              <span className="relative flex shrink-0 overflow-hidden rounded-full w-8 h-8" 
                ...
                <div className="rounded-full bg-gray-100 border p-1">
                  {m.role === 'user' ? (
                    <img alt="image" src="/user.png" />
                  ) : (
                    <img alt="image" src="/bot.png" />
                  )}
                </div>
              </span>
