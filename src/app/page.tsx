...
export default function IdentityPrismWorkspace() {
  ...
  const performAudit = async () => {
    ...
    try {
      ...
      setAuditReport(data);
      toast.success('Graph Audit Complete');
    } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-[#FDFDFB] text-[#1A1A1A] flex flex-col selection:bg-accent/20 transition-colors duration-1000 overflow-x-hidden">
      <ToastContainer 
        position="bottom-right" 
        theme="light" 
        toastClassName="!bg-white !border !border-[#E0E0DE] !rounded-2xl !backdrop-blur-3xl !text-[#1A1A1A]" 
      />
      
      {/* Porcelain-Compatible Adaptive Background */}
            ...
            <AnimatePresence mode="wait">
                ...
                >
                  ...
                  <section className="space-y-6">
                    <div className="flex items-center justify-between px-2">
                       <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-text-tertiary ml-2 kinetic-text">Diagnostic_Telemetry</h2>
                       <button 
                         onClick={performAudit}
                         className="px-5 py-2 rounded-full bg-accent text-[10px] font-black text-bg-primary uppercase tracking-[0.2em] shadow-xl shadow-accent/20 hover:scale-105 transition-all active:scale-95"
                       >
                         Trigger_Audit
                       </button>
                    </div>
                    {auditReport && (
                      <div className="glass-panel p-8 rounded-[2rem] border border-accent/20 bg-accent/[0.02]">
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                           <div className="space-y-1.5">
                              <span className="text-[9px] font-black text-text-tertiary uppercase tracking-widest">Graph Integrity</span>
                              <div className="text-sm font-black text-accent">{auditReport.passed}/20 PASS</div>
                           </div>
                           <div className="space-y-1.5">
                              <span className="text-[9px] font-black text-text-tertiary uppercase tracking-widest">Isolation</span>
                              <div className="text-sm font-black text-success">LOCKED</div>
                           </div>
                           <div className="space-y-1.5">
                              <span className="text-[9px] font-black text-text-tertiary uppercase tracking-widest">Relationships</span>
                              <div className="text-sm font-black text-text-primary">{auditReport.metrics?.relationships_created || 0} ACTIVE</div>
                           </div>
                           <div className="space-y-1.5">
                              <span className="text-[9px] font-black text-text-tertiary uppercase tracking-widest">Concurrency</span>
                              <div className="text-sm font-black text-warning">STABLE</div>
                           </div>
                        </div>
                      </div>
                    )}
              ...
              {currentView === 'L3' && (
                ...
                >
                  ...
                  <section className="space-y-6">
                    <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-text-tertiary ml-2 kinetic-text">Public_Identity_Synthesis</h2>
                    <IdentityShowcase />
                  </section>
              ...
              {/* L4: COGNITIVE — Decision Layer (Autonomous Agent) */}
              {currentView === 'L4' && (
                <motion.div 
                  key="L4"
                  ...
                  className="space-y-8"
                >
                  <div className="glass-panel rounded-[3rem] overflow-hidden border border-primary/20 shadow-3xl">
                     <CognitiveConsole />
                  </div>
                </motion.div>
              )}
