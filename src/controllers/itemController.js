...
async function uploadFile(req, res) {
  ...
  try {
    const existing = await Item.findOne({ 'source.url': fileUrl });
    if (existing) {
      return res.status(200).json({ status: 'exists', item: hideVersionHistory(existing) });
    }
    ...
    const item = await Item.create({
      ...
      versioning: {
        current_hash: Item.contentHash(raw),
        previous_versions: []
      }
    });

    return res.status(201).json({ status: 'saved', file_url: fileUrl, item: hideVersionHistory(item) });
  } catch (error) {
    if (error.code === 11000) {
      return res.status(409).json({ error: 'duplicate source url or external_id' });
    }
...
async function ingestEmail(req, res) {
