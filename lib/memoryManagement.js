...
export async function ingestManagedPacket(req) {
  ...
  await updateStore((next) => {
    ...
    if (!existingSource) next.source_registry.push(createBaseDoc('src', source));
...
export async function reclassifyData(req) {
  ...
  await updateStore((next) => {
    ...
    const canMutatePacket = user.role === 'admin' || packet.owner?.user_id === String(user._id);
    if (!canMutatePacket) throw fail('forbidden', 'auth_error', 403);
    if (type) packet.type = PACKET_TYPES.has(type) ? type : packet.type;
    if (priority) packet.priority = normalizedPriority(priority);
    if (ownership_user_id && user.role === 'admin') packet.owner.user_id = String(ownership_user_id);
