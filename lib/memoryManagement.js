...
export async function reclassifyData(req) {
  ...
  await updateStore((next) => {
    ...
    const isOwner = packet.owner?.user_id === String(user._id);
    if (!isAdmin && !isOwner) throw fail('forbidden', 'auth_error', 403);
    if (type) packet.type = PACKET_TYPES.has(type) ? type : packet.type;
    if (priority) packet.priority = normalizedPriority(priority);
    if (ownership_user_id && isAdmin) packet.owner.user_id = String(ownership_user_id);
