...
export async function syncItem(req) {
  ...
  if (!item) {
    ...
    await enqueuePacketForProcessing(created._id, { reason: 'sync_created' });
  ...
  await enqueuePacketForProcessing(item._id, { reason: changed ? 'sync_updated' : 'sync_observed', force: changed });
