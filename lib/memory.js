  ...
  await updateStore((store) => {
    ...
    if (!source) {
      source = createBaseDoc('src', {
        ...
        mode: sanitizeString(explicit.mode || req.body?.ingestion_mode || 'manual'),
        frequency: sanitizeString(explicit.frequency || 'on_demand'),
  ...
  await updateStore((store) => {
    ...
    if (!row) {
      row = createBaseDoc('src', {
        ...
        mode: sanitizeString(mode || 'manual'),
        frequency: sanitizeString(frequency || 'on_demand'),
