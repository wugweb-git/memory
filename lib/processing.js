...
function removeDerivedPacketData(store, packetId) {
  const normalizedPacketId = String(packetId);
  store.signals_activity = store.signals_activity.filter((row) => String(row.packet_id) !== normalizedPacketId);
  store.signals_domain = store.signals_domain.filter((row) => String(row.packet_id) !== normalizedPacketId);
  store.signals_intent = store.signals_intent.filter((row) => String(row.packet_id) !== normalizedPacketId);
  store.signals_momentum = store.signals_momentum.filter((row) => String(row.packet_id) !== normalizedPacketId);
  store.relationships_graph = store.relationships_graph.filter((row) => String(row.packet_id) !== normalizedPacketId);
  store.embeddings = store.embeddings.filter((row) => String(row.packet_id) !== normalizedPacketId);
}
...
async function processSingleJob(store, job) {
  ...
  if (sensitivity === 'restricted') {
    removeDerivedPacketData(store, packetId);
    const processed = {
      packet_id: packetId,
...
export async function getSignalSummaries(options = {}) {
  const { user } = options;
  const store = await readStore();
  const scopedPacketIds = new Set(
    store.items
      .filter((item) => !item.archived && (!user || canAccessPacket(item, user)))
      .map((item) => String(item._id))
  );

  const scopedDomains = store.signals_domain.filter((row) => scopedPacketIds.has(String(row.packet_id)));
  const scopedActivity = store.signals_activity.filter((row) => scopedPacketIds.has(String(row.packet_id)));
  const scopedIntent = store.signals_intent.filter((row) => scopedPacketIds.has(String(row.packet_id)));
  const scopedProcessed = store.processed_packets.filter((row) => scopedPacketIds.has(String(row.packet_id)));
  const scopedJobs = store.processing_jobs.filter((job) => scopedPacketIds.has(String(job.packet_id)));

  const domainDistribution = {};
  for (const row of scopedDomains) {
    const key = row.primary_domain || 'general';
    domainDistribution[key] = (domainDistribution[key] || 0) + 1;
  }

  const activityHeatmap = scopedActivity
  ...
  for (const row of scopedIntent) {
    const key = row.intent || 'explore';
    intentDistribution[key] = (intentDistribution[key] || 0) + 1;
  }

  return {
    processed_packets: scopedProcessed.length,
    queued_packets: scopedJobs.filter((j) => j.state === 'pending').length,
    failed_packets: scopedJobs.filter((j) => j.state === 'failed').length,
