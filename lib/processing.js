...
export async function getSignalSummaries() {
  ...
  return {
    processed_packets: store.processed_packets.length,
    queued_packets: store.processing_jobs.filter((j) => j.state === 'pending').length,
    failed_packets: store.processing_jobs.filter((j) => j.state === 'failed').length,
...
export async function listProcessingSignals(req) {
  const user = await getAuthUser(req);
  if (!user) throw fail('unauthorized', 'auth_error', 401);
  const summary = await getSignalSummaries();
