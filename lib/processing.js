...
function canAccessPacket(user, packet) {
  if (!user || !packet) return false;
  return user.role === 'admin' || packet.owner?.user_id === String(user._id);
}
...
async function processSingleJob(store, job) {
  ...
  if (sensitivity === 'restricted') {
    ...
    store.relationships_graph = store.relationships_graph.filter((row) => String(row.packet_id) !== packetId);
...
export async function runProcessingQueue({ batchSize = 25, user = null } = {}) {
  const result = { processed: 0, failed: 0, skipped: 0, jobs_total: 0 };

  await updateStore(async (store) => {
    const now = Date.now();
    const allowedPacketIds = user && user.role !== 'admin'
      ? new Set(store.items
        .filter((item) => !item.archived && item.owner?.user_id === String(user._id))
    ...
    const candidates = store.processing_jobs
      ...
      .filter((job) => !allowedPacketIds || allowedPacketIds.has(String(job.packet_id)))
      .slice(0, batchSize);
...
export async function getSignalSummaries(user = null) {
  ...
  const allowedPacketIds = user && user.role !== 'admin'
    ? new Set(store.items
      ...
      .map((item) => String(item._id)))
    : null;
  const isAllowed = (packetId) => !allowedPacketIds || allowedPacketIds.has(String(packetId));
  const domainDistribution = {};
  for (const row of store.signals_domain.filter((entry) => isAllowed(entry.packet_id))) {
  ...
  const activityHeatmap = store.signals_activity
    .filter((row) => isAllowed(row.packet_id))
  ...
  for (const row of store.signals_intent.filter((entry) => isAllowed(entry.packet_id))) {
    const key = row.intent || 'explore';
    intentDistribution[key] = (intentDistribution[key] || 0) + 1;
  }

  return {
    processed_packets: store.processed_packets.filter((row) => isAllowed(row.packet_id)).length,
    queued_packets: store.processing_jobs.filter((j) => j.state === 'pending' && isAllowed(j.packet_id)).length,
    failed_packets: store.processing_jobs.filter((j) => j.state === 'failed' && isAllowed(j.packet_id)).length,
...
export async function enqueueProcessingFromRequest(req) {
  ...
  if (!canAccessPacket(user, packet)) throw fail('forbidden', 'auth_error', 403);
...
export async function runProcessingFromRequest(req) {
  ...
  const result = await runProcessingQueue({ batchSize, user });
