...
import { normalizeScope } from './validation.js';
import { enqueuePacketForProcessing } from './processing.js';
...
export async function getItems(req) {
  ...
  const scope = normalizeScope(sanitizeString(req.query?.scope || 'mine'));
...
export async function createItem(req) {
  ...
  const item = createBaseDoc('itm', {
    ...
    source: {
      type: sanitizeString(source_type || 'manual'),
      origin: sanitizeString(source_origin || 'manual'),
      url,
    ...
    content_chunk: sanitizeString(content_chunk || cleanedRaw),
  ...
  await enqueuePacketForProcessing(item._id, { reason: 'item_created' });
