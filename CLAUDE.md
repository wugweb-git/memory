...
## Architecture (layers)
```
L0 Blob (intake buffer, Mongo)      → L1 Memory (Mongo + RAG)
L2 Signals (Mongo) → L2.5 Semantic graph (Mongo)
L3 Cognitive "brain" (Neon/Postgres) → L4 Persona (Neon) → L5 Execution/publish (Neon)
```
...
- **Two Prisma clients — NEVER mix:**
  - `import { mongo } from '@/lib/db/mongo'` → L0–L2.5 (schema `prisma/mongo/schema.prisma`, generated `src/generated/mongo`)
  - `import { postgres } from '@/lib/db/postgres'` → L3–L5 (schema `prisma/postgres/schema.prisma`, generated `src/generated/postgres`)
  - Regenerate both: `npm run prisma:generate`
- LangChain + OpenAI (gpt-4o-mini / gpt-4o), **Langfuse** observability, **Sanity** (public showcase CMS), Vercel Blob.
...
## The brain (L3)
Pipeline (`src/lib/cognitive/orchestrator.ts`): `buildContext → buildPrompt → LLM → sanitize → dedup → critic → logDecisionNeon`, Langfuse-traced. Routes: `/api/cognitive/{decide,evaluate,gaps,prioritize,feedback,history,traces}`. Reads L1–L2.5 (Mongo), writes logs to Neon, **never mutates lower layers**. Modes: architect/founder/operator.
